...
object TR {
  val edit_message = TypedResource[android.widget.EditText](R.id.edit_message)
  val table = TypedResource[android.widget.TableLayout](R.id.table)

  object layout {
    val list = TypedLayout[android.widget.TextView](R.layout.list)
    val main = TypedLayout[android.widget.LinearLayout](R.layout.main)
    val table = TypedLayout[android.widget.ScrollView](R.layout.table)
  }
}
